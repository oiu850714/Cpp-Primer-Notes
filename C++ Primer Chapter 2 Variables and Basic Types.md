...
* 最後給了一個 advice：**AVOID UNDEFINED AND IMPLEMENTATION-DEFINED BEHAVIOR**
    ...
    * UB 跟 ID 的差別：
        * 一個是標準完全沒有給定義
        * 一個是標準有給定義，但是他是"給一個(最小)範圍"，compiler 實作在這個標準範圍都可以，可是你就TMD 不能假設你的程式就運作在這個範圍的某個點上，你只能假設你的程式在標準給定的最小範圍內會 work。
...
## 2.2 Variables
    variable 就是有名稱的儲存空間
    每一個 variable 都有型別
    variable 的型別決定的 variable 使用的 memory 的 layour(擺設) 以及 size(大小)，variable 可以儲存的 value 的範圍，還有可以對這個 variable 所進行的操作(operations)
    有時候會把 variables 稱作 objects
...
* Initializers
    * 嚇人的東西...
    * Initialization in C++ is a surprisingly complicated topic and one we initialize 所有的 pointerswill return to again and again. 
...
* Default Initialization
    * 當你在宣告變數時沒有指定 initializer，那個變數會使用那個型態定義的 defualt value 做初始化
        * 額外補充，如果你變數的型別是 C++ 標準物件，那都有一些很好的 default value，但如果你的型別是 built-in type，那就要看**你是在哪個地方宣告這個變數的**，如果在全域，預設是 0，如果在 local，可愛的 C(綠)姊姊會不幫你初始化，變數內容是垃圾 QQ
...
* **A definition creates the associated entity.**
    * 相信我，上面的這兩行對新手甚至是學過交大那悲劇的 OOP 的人來說一定是抽象到靠北
    ...
    * Any declaration that includes an explicit initializer is a definition
        * 一個宣告如果有帶 initializar，那他也是 definition
...
* 這也是超重要議題
    * 我怎麼覺得每個都很重要...
    * Scope 就是整個程式的程式碼的某些部分，某個 name 有特別的意義
    ...
    * 用下面這個例子來說明 scope:
        ...
        #include <iostream>
        int main() {
          int sum = 0; // sum values from 1through 10inclusive
        ...
        * sum 有 block scope
        * val 也有 block scope，不過他的 scope 在 sum 的 scope 的裡面
* Nested Scope
    * Scope 可以包含其他 Scope
...
* 前面說過，變數宣告是由一個型別名稱加上一堆的變數名稱形成的
    ...
    *   Each declarator names a variable and gives the variable a type that is **related** to the base type.
        ```C++
            type_name var_name;
        ```
*   在前面的例子，我們的 declarator 就僅僅只是變數名稱而已；這樣的 declarator 給予變數名稱的型別就是型別名稱(type_name)
*   但之後會介紹到，我們可以使用更複雜的 declarator，讓我們的變數名稱的型別變成 compound type，而這個 compound type 是建構在 type_name 上的
### 2.3.1 References
...
* 注意，reference type 一定要初始化!
    ```C++
    int ival = 1024; 
    int &refVal = ival; // refValrefers to (is another name for) ival 
...
* 當 reference 被 bound 到某物件之後，你對 reference 做的任何操作就會真的發生在 reference bound 的物件上
* 然後我們不能定義一種 type 叫做 reference to reference to what，因為 reference 不是物件；之後要說的 pointer 是可以的。
...
之前有說過，宣告的時候，declarator 的型態一定要跟 type_name 一致，不過有例外，其中一個例外就是 const reference(reference to const)
...
* 而 reference 型別則指有 Low-Level const，因為 reference 本身不是物件，不是物件根本沒有是否為 const 的問題；但他一定得 bind 在某個物件上，所以他有 Low-Level const
...
* 但是 Low-Level const 就不會被無視惹
    * 沒 const 的可以丟給 const，可是有 const 的不可以丟給沒 const 的(?
...
* Example
    ...
    ```
    * 上面第三個跟第四個例子，尤其是第四個，好好理解之後會對 const exp 更有感覺
#### constexpr functions
    * 第六章會講
* **BEST PRACTICE**
...
* Literal Types
    ...
    * reference 跟 pointer 要能成為 constexpr，只能去 bind 或指向某種 variables。
        ...
        * 指標還有一點要注意：
        ...
        ```
        * constexpr 是作用在Top-level(也就是指標上)的，而不是 Low-level(也就是說指標指向的型別)。
...
* 傳統的 typedef
    ...
    * Declarations that include typedef define type aliases rather than variables.
        * 如果你的宣告的 base 裡面有 typedef，這樣就是在定義 type aliases，而不是一個新的 object
        * 這時候 declarator 一樣可以包含 type modifier(*, & 之類的)
* C++11 的新方式，叫做 alias declaration
    ```C++
    using SI = Sales_item; // SIis a synonym for Sales_item
...
* C++11 的 auto
    ...
    * 注意：The type that the compiler infers for auto is **not always exactly the same as** the initializer’s type. 
        ...
        * 而且如果你給的 initializer 實際上是 referece 的話，auto 會用 referece bind 的 object 來推斷型別
        ...
        auto b = ci; // bis an int(top-level constin ciis dropped) 
        auto c = cr; // c is an int(cr is an alias for ciwhose constis top-level) 
        auto d = &i; // dis an int*(&ofan intobject is int*)
        auto e = &ci; // e is constint*(&of a const object is low-level const)
        ```
    ...
    * 注意：When we ask for a reference to an auto-deduced type, **top-level constsin the initializer are not ignored.**
        * initializer 的 top-level 之後就會變成 reference 的 low-level
### 2.5.3 The decltype Type Specifier
另一種類似 auto 的東西
使用方式:
* decltype(operand) var_name = initializer;
* Compiler 會去分析 operand 的 type，**但是不會計算 operand**
* Theway decltype handles top-level const and references differs subtly from the way auto does.
